...
def _parse_str(s: str):
    """
    如果 s 能解析为 int 就返回 int；
    否则若能解析为 float 就返回 float；
    否则返回原始字符串 s（保留原始空白与大小写）。
    """
    t = s.strip()
    try:
        return int(t)
...
def parse_str(obj, *, parse_dict_keys: bool = False):
    """
    递归解析对象：
      - str: 按规则尝试转成 int/float，否则原样返回
      - list/tuple/set: 递归解析内部元素，返回同类型容器
      - dict: 默认只解析 value；若 parse_dict_keys=True，则连 key 一起解析
      - 其它类型：原样返回
    注意：不处理带环引用的容器。
    """
    if isinstance(obj, str):
        return _parse_str(obj)

    if isinstance(obj, list):
        return [parse_str(x, parse_dict_keys=parse_dict_keys) for x in obj]

    if isinstance(obj, tuple):
        return tuple(parse_str(x, parse_dict_keys=parse_dict_keys) for x in obj)
    ...
    if isinstance(obj, dict):
        if parse_dict_keys:
            return {
                parse_str(k, parse_dict_keys=parse_dict_keys): parse_str(v, parse_dict_keys=parse_dict_keys)
                for k, v in obj.items()
            }
        else:
            return {k: parse_str(v, parse_dict_keys=parse_dict_keys) for k, v in obj.items()}
