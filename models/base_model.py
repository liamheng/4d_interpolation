...
class BaseModel(ABC):
    ...
    def __init__(self, opt):
        ...
        self.image_paths = []
        if not opt.is_train:
            self.metrics = MyMetrics.get_metrics(opt.task)(opt, self.device)
    ...
    def load_networks(self, load_filename_list):
        ...
        for load_filename, model_name in zip(load_filename_list, self.model_names):
            ...
            if isinstance(net, torch.nn.DataParallel):
                net = net.module
            state_dict = torch.load(load_filename, map_location=str(self.device), weights_only=False)
    ...
    @torch.no_grad()
    def test(self, compute_visuals=True):
    ...
    def optimizers_step_integration(self):
        if self.opt.grad_accum is None or (self.opt.current_iteration % self.opt.grad_accum == 0):
            if self.opt.clip_norm is not None:
                params = []
                for net_name in self.model_names:
                    ...
                    for p in net.parameters():
                        if p.requires_grad and p.grad is not None:
                            params.append(p)
                if len(params) > 0:
                    clip_grad_norm_(params, max_norm=float(self.opt.clip_norm))
    ...
    def update_learning_rate(self):
        ...
        Default behavior (no warmup): called at the end of every epoch.
        When opt.warmup_percentage > 0, train.py will additionally call this at the end of iterations
        during warmup by setting opt.lr_update_mode = "iter".
        """
        # Use opt as global context; train loop may set this to "iter" during warmup.
        if not hasattr(self.opt, 'lr_update_mode'):
            self.opt.lr_update_mode = 'epoch'
        ...
        # Avoid flooding logs during iteration-based warmup
        if getattr(self.opt, 'lr_update_mode', 'epoch') == 'epoch':
